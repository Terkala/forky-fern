...
public sealed class WizardFamiliarTest
{
    ...
    public async Task SummonCreatesFamiliarWithCorrectComponents()
    {
        ...
        await server.WaitPost(() =>
        {
            ...
            var summonAction = actionSys.GetAction(summonActionId.Value);
            Assert.That(summonAction, Is.Not.Null);
            actionSys.SetUseDelay(summonActionId.Value, null);
            actionSys.PerformAction((wizard, null), summonAction!.Value);
        });
    ...
    public async Task FamiliarDeathTriggersCooldown()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            var actionComp = entMan.GetComponent<ActionComponent>(summonActionId!.Value);
            Assert.That(actionComp.Cooldown, Is.Not.Null, "Summon action should have cooldown after familiar death");
            Assert.That(actionComp.Cooldown!.Value.End, Is.GreaterThan(timing.CurTime));
        });
    ...
    public async Task TeleportActionMovesFamiliarToWizard()
    {
        ...
        await server.WaitPost(() =>
        {
            var teleportAction = actionSys.GetAction(teleportActionId);
            Assert.That(teleportAction, Is.Not.Null);
            actionSys.SetUseDelay(teleportActionId, null);
            actionSys.PerformAction((familiar, null), teleportAction!.Value);
        });
    ...
    [Test]
    public async Task TeleportActionMovesFamiliarToWizardCrossMap()
    {
        await using var pair = await PoolManager.GetServerClient(new PoolSettings
        {
            Dirty = true,
        ...
        await server.WaitPost(() =>
        {
            wizard = entMan.SpawnEntity("MobHuman", mapA.GridCoords);
            var summonActionId = actionSys.AddAction(wizard, "ActionSummonMiniDragon");
            Assert.That(summonActionId, Is.Not.Null);
            var summonAction = actionSys.GetAction(summonActionId.Value);
            Assert.That(summonAction, Is.Not.Null);
        ...
        await server.WaitPost(() =>
        {
            var wizardMapCoords = new MapCoordinates(new Vector2(1, 1), mapB.MapId);
            transformSys.SetMapCoordinates(wizard, wizardMapCoords);
        });
        ...
        await server.WaitAssertion(() =>
        {
            var wizardXform = entMan.GetComponent<TransformComponent>(wizard);
            var familiarXform = entMan.GetComponent<TransformComponent>(familiar);
            Assert.That(familiarXform.MapID, Is.EqualTo(wizardXform.MapID), "Familiar should be on same map as wizard after cross-map teleport");
            Assert.That(familiarXform.MapID, Is.EqualTo(mapB.MapId), "Familiar should be on map B");
            var wizardPos = wizardXform.Coordinates;
            var familiarPos = familiarXform.Coordinates;
            Assert.That((familiarPos.Position - wizardPos.Position).Length(), Is.LessThan(2f), "Familiar should be near wizard after cross-map teleport");
        });
