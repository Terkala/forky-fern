...
public abstract partial class CESharedSkillSystem : EntitySystem
{
    ...
    /// <summary>
    /// Resolves CE skill storage on the entity or, for mind-bound progression (e.g. mages), on its mind.
    /// </summary>
    public bool TryGetSkillStorage(EntityUid actor, out Entity<CESkillStorageComponent> storage)
    {
        if (_skillStorageQuery.TryGetComponent(actor, out var onActor))
        {
            storage = new Entity<CESkillStorageComponent>(actor, onActor);
            return true;
        }

        if (_mind.TryGetMind(actor, out var mindId, out _) && _skillStorageQuery.TryGetComponent(mindId, out var onMind))
        {
            storage = new Entity<CESkillStorageComponent>(mindId, onMind);
            return true;
        }
    ...
    private void OnInteracted(Entity<CESkillPointConsumableComponent> ent, ref UseInHandEvent args)
    {
        ...
        if (_net.IsServer)
        {
            ...
            if (TryGetSkillStorage(args.User, out var skillStorage))
                TryAddSkillPoints(skillStorage.AsNullable(), ent.Comp.PointType, collect);
        }
